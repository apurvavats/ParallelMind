...
export default function HistoryDetail() {
  ...
  const fetchProjectDetails = async () => {
    try {
      ...
      // 🚀 CALL BACKEND (Route we just made)
      const res = await axios.get(`${VITE_API_URL}/history/${id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });

      const { project, variants } = res.data;
  ...
  return (
    <div className="min-h-screen pt-24 pb-20 text-white bg-[#0F071A]">
      <Header />
