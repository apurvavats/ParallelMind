...
import { VITE_API_URL } from "../config"; // 👈 Import Config
import { useAuth } from "../context/AuthContext"; 
import { Loader2, Calendar, ChevronRight, Search } from 'lucide-react';
import Header from "../components/Header";
import Footer from "../components/Footer";
...
export default function History() {
  const { user, token } = useAuth(); // 👈 Make sure you get 'token' from context
  const [history, setHistory] = useState([]);
  const [loading, setLoading] = useState(true);
  ...
  const fetchHistory = async () => {
    try {
      ...
      // 🚀 CALL BACKEND instead of Supabase directly
      const res = await axios.get(`${VITE_API_URL}/history`, {
  ...
  // ... (Keep your loading check and return JSX exactly as it was)
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#0F071A] text-white">
        <Loader2 className="animate-spin h-8 w-8 text-purple-500" />
  ...
  return (
    <div className="min-h-screen text-white pt-24 bg-[#0F071A]">
      <Header />
